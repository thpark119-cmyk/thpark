...
export const translations = {
  ko: {
    ...
    admin: {
      ...
      backfillNotice1: '이미 용량 정보가 있는 파일은 제외됩니다.',
      backfillNotice2: '이번 단계(1단계)에서는 실제 Firestore 사용자 원본 문서를 수정하지 않으며, 관리자 전용 cache 컬렉션도 생성하지 않습니다.',
      backfillNotice3: 'Storage metadata를 조회할 수 있는 파일(성공)만 다음 단계에서 실제로 보정할 수 있습니다.',
  ...
  en: {
    ...
    admin: {
      ...
      backfillTitle: 'Legacy File Storage Metadata Backfill (Phase 1: Scan & Preview)',
      ...
      backfillNotice1: 'Files that already have valid size and content type information are excluded.',
      backfillNotice2: 'In this phase (Phase 1), we do not modify the original user documents or create any admin cache collections.',
      backfillNotice3: 'Only files for which Storage metadata can be successfully fetched can be backfilled in the next phase.',
  ...
  de: {
    ...
    admin: {
      ...
      backfillTitle: 'Speicherplatz-Metadaten-Korrektur (Phase 1: Scan & Vorschau)',
      ...
      backfillNotice1: 'Dateien, die bereits gültige Größeninformationen haben, werden ausgeschlossen.',
      backfillNotice2: 'In dieser Phase (Phase 1) werden die Originaldokumente der Benutzer nicht geändert und es werden keine Admin-Cache-Sammlungen erstellt.',
      backfillNotice3: 'Nur Dateien, deren Metadaten erfolgreich abgerufen werden können, können in der nächsten Phase korrigiert werden.',
