...
본 문서는 Music In One / Mio의 관리자 계정(`thpark119@gmail.com`)이 과거 누락된 파일의 용량 및 미디어 포맷 metadata(`getMetadata`)를 스캔하여 복구할 수 있도록 보장하는 Firebase Storage Rules 설정 초안입니다.
...
2. **관리자 읽기 전용 권한 부여**: 백필 스캔 기능을 정상 수행하기 위해 지정된 관리자 계정(`thpark119@gmail.com`)에 한해 전체 `users/{userId}/**` 파일들의 **metadata 조회(`getMetadata`) 및 읽기(read)** 권한만을 허용합니다.
3. **쓰기 및 삭제 방지**: 관리자 권한을 가진 계정이라 하더라도 타인의 파일을 강제로 덮어쓰거나(write) 임의로 영구 삭제(delete)하는 파괴적 액션은 차단되어 안전성을 극대화합니다.
...
service firebase.storage {
  match /b/{bucket}/o {
    ...
    match /users/{userId}/{allPaths=**} {
      ...
      // 2. 관리자: 타 사용자의 파일 스캔(Metadata 확인)을 위해 읽기 권한(get, list)만 허용
...
* **효과**: 위 규칙이 배포되면 관리자 대시보드 내의 **“기존 파일 용량 보정 (1단계: 스캔 및 미리보기)”** 기능 실행 시 타 사용자의 Storage에 업로드된 파일의 크기(size) 및 파일 타입(contentType) 조회가 완전히 허가되어, `'permission-denied'` 오류 없이 안전하게 스캔 결과를 미리볼 수 있게 됩니다.
